...
public class ClientEntityAnimator : EntityAnimatorComponent<EPlayerMovementAnimations>
{
    ...
    {
        if (properties.Contains(nameof(CurrentAnimation)))
        {
            EPlayerMovementAnimations previousAnimation = CurrentAnimation;
            CurrentAnimation = (EPlayerMovementAnimations)properties[nameof(CurrentAnimation)];
            Console.WriteLine($"[EntityAnimator.OnStateChange] {previousAnimation} -> {CurrentAnimation}");
        }
    }
}